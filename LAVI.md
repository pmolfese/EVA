# Rhythmicity Explorer: LAVI, ABBA, WTPL, and Burst Analysis

## Status

Detailed implementation plan. The EEG-menu route and an explicitly nonfunctional
Rhythmicity Explorer navigation shell are implemented. Milestones 0–2 now provide
the pinned numerical reference and on-demand paper-valid significance. Milestone
3 provides the production CPU backend; the user-facing result workspace remains
gated on Milestone 4.
...
The reference repositories contain MATLAB and Python implementations, but EVA's executable numerical oracle is the pinned LAVI Python implementation only. MATLAB files remain provenance and historical-comparison material; they are not required to build or regenerate milestones 1–5. EVA will not launch Python or MATLAB at runtime.
...
The **Bands** mode is the first shippable product. Event-related WTPL follows. Burst analysis is a later milestone and must not block LAVI/ABBA.
...
Later milestones may add independently generated IAAFT and WTPL fixtures. WTPL
does not receive an executable fixture until a validated Python oracle exists.
...
Do not block the first correct release on Metal. After CPU FFT validation, add one of:

1. a coefficient-tile kernel followed by LAVI/WTPL reduction kernels; or
2. a fused kernel that computes only the necessary lagged coefficients and accumulators.

Avoid a full all-channel complex cube. For a 128-channel, 10-minute, 1-kHz recording, it is prohibitively large. Tile by channel and frequency, and keep memory below the same conservative cap used by the existing TF Metal backend.

GPU acceptance requires:

- CPU reference parity within a documented float tolerance;
- identical band boundaries for validated fixtures;
- no significance flip except within a documented boundary tolerance;
- automatic CPU fallback;
- progress and cancellation behavior matching the CPU path.
...
WTPL is implemented inside the Rhythmicity module and rendered in the Event-related mode. After validation, the existing Time-Frequency measure picker may gain a **WTPL** shortcut that calls the same engine. There must be only one numerical implementation.
...
Burst analysis is milestone 7, after LAVI/ABBA and WTPL are stable.
...
## 22. Milestones and work breakdown
...
- [ ] Add `RhythmicityExplorerViewModel`.
- [ ] Wire it into `WaveformView` as a standalone explorer.
- [ ] Add source, segment, and channel-scope controls.
- [ ] Build LAVI spectrum with median/ribbon/band overlays.
- [ ] Build ABBA band table.
- [ ] Add selected-channel and multi-channel summaries.
- [ ] Add validity warnings and stale-result behavior.
- [ ] Add CSV/NPY/JSON package export.
- [ ] Add Help and citation copy.
- [ ] Add release notes.
...
- [ ] Add recording-scoped `DetectedRhythmicityBandSet` state.
- [ ] Add explicit “Use in Time-Frequency” action.
- [ ] Add TF band-source picker.
- [ ] Overlay ABBA boundaries and peaks on TF maps.
- [ ] Use selected ABBA bands for TF ROI scalars.
- [ ] Preserve global band preferences unless explicitly saved.
- [ ] Export band-source provenance with TF results.
- [ ] Add invalidation and integration tests.
...
- [ ] Paper-preset LAVI matches the pinned Python reference within documented tolerance.
- [ ] ABBA borders, peaks, directions, alpha-relative identities, and significance match fixtures.
- [ ] WTPL matches its validated independent Python oracle.
- [ ] Segment/trial boundaries are never crossed.
- [ ] Invalid/insufficient data are distinct from zero or nonsignificant results.
- [ ] Surrogate significance is reproducible and configuration-matched.

### Product behavior

- [ ] Rhythmicity Explorer works without requiring epochs for Bands mode.
- [ ] Event-related mode clearly requires and uses trials.
- [ ] Users can inspect single-channel and multi-channel results.
- [ ] ABBA bands can be explicitly adopted by Time-Frequency.
- [ ] Global band defaults are unchanged unless the user explicitly saves a preset.
- [ ] Results become visibly stale when upstream signal processing changes.
- [ ] Long runs are cancellable and never publish partial results as complete.
...
- [ ] Exports include axes, configuration, source selection, processing history, valid counts, warnings, software version, backend, seed, and lookup checksum.
- [ ] Reference fixture generation is documented.
- [ ] Help and release notes cite the paper and toolboxes.
...
The first production release should include milestones 0–5. WTPL is the next independently valuable release. Burst analysis and Metal acceleration follow based on profiling and user demand.
