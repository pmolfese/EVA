...
struct EEGAnalysisSheet: View {
    ...
    private func segmentRow(_ decision: EEGAnalysisSegmentDecision) -> some View {
        HStack {
            ...
            Text(decision.artifactOverlapFraction.map(formatPercent) ?? "—")
                .foregroundStyle(decision.artifactOverlapFraction == nil ? Color.secondary : Color.primary)
                .help(decision.artifactOverlapFraction == nil ? "Labeled artifacts were not assessed: no artifact source was included in this analysis." : "")
