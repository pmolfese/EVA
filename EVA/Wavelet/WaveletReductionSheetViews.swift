...
extension WaveformView {
    ...
    func runWaveletReduction(on input: MFFSignalData) {
        ...
        waveletReductionTask = Task {
            await applyWaveletReduction(to: input)
    ...
    /// Awaitable dispatcher shared by the interactive Run/Re-run button
    /// (wrapped in a Task above) and the replay coordinator (awaited) — one
    /// code path, via `WaveletReductionViewModel.apply(to:excludedChannels:
    /// analysisBand:onApplied:)`. This wrapper's job is just the
    /// interactive-only session guard on the cross-domain invalidation
    /// closure — see `applyGradientCorrection`'s doc comment for why.
    func applyWaveletReduction(to signal: MFFSignalData) async {
    ...
    func artifactTemplateConfiguration(
        ...
        ArtifactTemplateConfiguration(
            ...
            mergeBehavior: template.mergeBehavior,
            ...
            // Continuous scan is gated to Ocular artifacts (see DefinedArtifactType);
            // force windowed for any other type even if the UI state is stale.
            topographyScanStyle: template.type == .ocular ? template.topographyScanStyle : .windowed,
            continuousMinDurationSeconds: max(template.continuousMinDurationSeconds, 0.01),
            continuousMaxDurationSeconds: max(template.continuousMaxDurationSeconds, 0),
            continuousSmoothingSeconds: max(template.continuousSmoothingSeconds, 0),
