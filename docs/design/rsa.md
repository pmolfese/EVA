...
This holds the interchange contract verified against `3dRSA` source, the two
traps that silently corrupt results, the estimator ladder, the scientific
hazards, and the `3dRSA`-side changes worth requesting. **Phases RS-1 … RS-9 and
their status are in `ROADMAP.md` § RSA; none are scheduled.**
