...
**Status lives in the roadmap, not here.** Milestones 0–5 shipped 2026-09-11
(see `ROADMAP_COMPLETE.md` § Time-Frequency & Rhythmicity); WTPL, bursts and
Metal acceleration are open (see `ROADMAP.md` § Time-Frequency & Rhythmicity).
...
EVA's executable numerical oracle is the pinned LAVI Python implementation only.
MATLAB files remain provenance and historical-comparison material. EVA will not
launch Python or MATLAB at runtime.
